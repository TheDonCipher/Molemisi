...
**Typography — Molemisi Pixel font** (per `docs/Font_Guide.png`): 16-bit RPG pixel display font, **PixelLab-generated** via `POST /generate-font-pro` (`scripts/generate-font.mjs`, Bold, 16px glyphs) — canonical file `assets/branding/font/MolemisiPixel-Bold.ttf` (82 glyphs, full A–Z/a–z/0–9/punctuation coverage, verified via cmap parse + atlas render). Synced into the web app by `scripts/build-font.mjs` (runs in `predev`/`prebuild`). Offline fallback: hand-drawn glyph bitmaps (`scripts/lib/glyphs.mjs`) compiled by the zero-dependency TTF compiler (`scripts/lib/ttf.mjs`) → `MolemisiPixel-Bold-handcrafted.ttf`; rebuild with `node scripts/build-font.mjs --rebuild --specimen`. Wired into the web app via `next/font/local` (`--font-molemisi-pixel`): `font-headline`/`font-headline-*` slots use it for titles, menus, dialogue and HUD; body text stays Rubik, data stays Space Mono. The Phaser game loads it via the FontFace API (`apps/game/src/fonts.ts`, boot gated on font readiness) with the same headline/body split.

**Font sizing (measured, `apps/game/public/font-probe.html`):** Canvas 2D always anti-aliases; the 16px-grid font renders crispest at ≥16px and needs a **12px minimum** for legible downsampled text (10–11px = 5–7px ink, too soft). All Phaser panel text is 12px+. An 8px-grid 'Molemisi Pixel Small' variant (`MolemisiPixel-Small.ttf`, `--px 8 --name Small`) was generated and measured **worse** than the 16px font at 12px (23% vs 27% crisp) — kept registered as `FONT_SMALL` for future use but **not used**; the 16px font at 12px is the best small-text rendering available.
